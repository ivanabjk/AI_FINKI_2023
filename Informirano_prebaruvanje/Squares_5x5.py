    ...
    def successor(self, state):
        ...
        for i in range(len(state)):
            ...
            if self.check_valid(squares):
                succ[f'Pomesti kvadratche {i + 1} gore'] = tuple(squares)

            squares[i] = (x, y - 1)
            if self.check_valid(squares):
                succ[f'Pomesti kvadratche {i + 1} dolu'] = tuple(squares)

            squares[i] = (x - 1, y)
            if self.check_valid(squares):
                succ[f'Pomesti kvadratche {i + 1} levo'] = tuple(squares)

            squares[i] = (x + 1, y)
            if self.check_valid(squares):
                succ[f'Pomesti kvadratche {i + 1} desno'] = tuple(squares)
