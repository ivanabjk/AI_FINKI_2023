...
if __name__ == "__main__":
    ...
    train_x2 = []
    for row in train_x:
        row2 = [row[i] for i in range(len(row)) if i != col_index]
        train_x2.append(row2)
    ...
    for i in range(len(entry)):
        if i != col_index:
            entry2.append(entry[i])

    predicted_class = classifier.predict([entry2])[0]

    print(f'Accuracy: {acc}')
    print(predicted_class)
    print(classifier.predict_proba([entry2])[0])
